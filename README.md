...
- **Fontes alternativas de áudio (AirPlay/Spotify Connect/Squeezelite) — avaliado e descartado por ora**: não existe componente ESP-IDF mantido pra AirPlay ou Spotify Connect (só aplicações completas de terceiros tipo esp-airsync/airplay-esp32/cspot, que exigiriam "vendorizar" o projeto inteiro, não uma dependência limpa). Mais importante: Wi-Fi e Bluetooth Classic simultâneos com áudio nos dois não é estável no ESP32 clássico (rádio único de 2.4GHz) — nenhum relato confiável de AirPlay(Wi-Fi)+A2DP(BT) concorrentes e estáveis no mesmo chip. Um modo alternativo exclusivo (Bluetooth OU AirPlay, não os dois ao mesmo tempo) seria tecnicamente viável, mas é um projeto grande à parte.
