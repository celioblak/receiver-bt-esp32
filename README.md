...
| POST | `/api/config` | Salva configurações (nome, Wi-Fi, timeout do relé, MQTT, `slim_host`, `ma_token`, `bt_discoverable`, `mic_enabled`, `pairing_lock`) |
...
## Entrada de microfone / Karaokê (sem teste em hardware)

Microfone onboard do ES8388 (par diferencial MIC1), desligado por padrão, ligável em Configurações (`mic_enabled`) ou via MQTT (precisa reiniciar pra valer). Quando ligado, o áudio do microfone é **misturado automaticamente por cima de qualquer coisa que esteja tocando** (Bluetooth ou DLNA) — é o único uso do mic hoje, "modo karaokê" sempre ativo enquanto o mic estiver ligado, sem toggle separado.

- **`main/es8388.c`**: `es8388_mic_init()`/`es8388_mic_deinit()`/`es8388_mic_set_gain()` ligam o bloco ADC do codec (registrador `ADCPOWER`, independente do `DACPOWER` usado pela reprodução) e selecionam a entrada MIC1. **Valores de registrador não validados em hardware real** — são a melhor referência disponível (datasheet ES8388 + convenção usada por outros projetos com essa placa, ex. squeezelite-esp32/ESP-ADF), a confirmar/ajustar (ganho do PGA em especial, `es8388_mic_set_gain()`) assim que houver como testar com sinal de áudio de verdade.
- **`main/audio_codec.c`**: o canal I2S é TX-only (só reprodução) por padrão. Com `mic_enabled` ligado, `audio_codec_init()` aloca também o canal RX — **num `i2s_new_channel()` separado do TX**, com um buffer de DMA bem mais raso (3 descritores de 128 amostras, ~1,5KB) que o do TX (12 descritores de 240 amostras, ~11,25KB, necessário pra não engasgar áudio de verdade durante coexistência Wi-Fi/Bluetooth) — o RX não precisa da mesma profundidade. O `i2s_std_config_t` (clock/slot/pinos) dos dois canais precisa ser **byte a byte idêntico** pra o driver reconhecer o par como full-duplex de verdade (checagem interna do IDF); só a config de DMA pode diferir. Decidido uma única vez no boot — não dá para ligar/desligar em runtime sem reiniciar.
- **Mixagem** (`write_with_mic_mix()` em `audio_codec.c`): `audio_codec_write()` — o ponto onde tanto `bt_audio.c` quanto `dlna_renderer.c` empurram PCM pro DAC — lê um bloco do microfone (timeout curto, nunca bloqueia a reprodução esperando o mic) e soma amostra a amostra por cima da música (saturando em vez de estourar, sem atenuação digital extra — o ganho fica no PGA analógico do ES8388). Fica centralizado ali de propósito: cobre Bluetooth e DLNA sem duplicar lógica em cada fonte. Buffers de mixagem em PSRAM (não RAM interna).
- Uso de RAM interna do mic reduzido de ~11-13KB pra menos de 2KB com a mudança acima (medido ao vivo) — antes disso, mic ligado + Bluetooth conectado derrubava a RAM interna a ponto do watchdog de rede reiniciar o dispositivo sozinho.
...
- **Entidades de controle** (bidirecionais, tópicos de comando próprios sob `homeassistant/receiver_bt/cmd/`, documentados aqui por não existir um schema oficial de `media_player` da HA que cubra bem `next`/`previous`):
  ...
  - `switch` Microfone (`cmd/mic_enabled`) — ver [Entrada de microfone / Karaokê](#entrada-de-microfone--karaokê-sem-teste-em-hardware): reflete o estado real (pós-boot), não a preferência salva; ligar/desligar via MQTT só tem efeito depois de reiniciar o dispositivo.
