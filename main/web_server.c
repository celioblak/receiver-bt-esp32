...
static esp_err_t api_mic_input_post(httpd_req_t *req)
{
    char buf[64];
    cJSON *root = recv_json_body(req, buf, sizeof(buf));
    if (root == NULL) {
    ...
    cJSON *pga = cJSON_GetObjectItem(root, "pga");
    if (cJSON_IsNumber(input)) {
        es8388_mic_set_input((uint8_t)input->valueint);
    }
    if (cJSON_IsNumber(pga)) {
        es8388_mic_set_gain(pga->valueint);
    }
    cJSON_Delete(root);
...
/* POST /api/mic/clock -- reprograma o clock do I2S (44100 -> 48000 -> 44100)
 * sem reiniciar o aparelho.
 *
 * Diagnostico do "ADC sobe chiando em ~1 boot de 5" (2026-08-30). Ja esta
 * provado que NAO e configuracao: os 54 registradores do codec sao identicos
 * entre um boot bom e um ruim, o ruido nao muda com a entrada selecionada nem
 * com o PGA, e nem o reset total do codec (0x02=0xFF) recupera -- so reiniciar
 * o ESP32. Se o clock que o ESP32 entrega for o que difere, reprograma-lo tem
 * de limpar o ruido sem reboot nenhum. Se nem isso limpar, a diferenca esta em
 * estado interno do codec que nenhuma escrita alcanca. */
static esp_err_t api_mic_clock_post(httpd_req_t *req)
{
    esp_err_t e1 = audio_codec_reconfigure_clock(48000);
    esp_err_t e2 = audio_codec_reconfigure_clock(44100);
    cJSON *resp = cJSON_CreateObject();
    cJSON_AddBoolToObject(resp, "ok", e1 == ESP_OK && e2 == ESP_OK);
    cJSON_AddStringToObject(resp, "ida", esp_err_to_name(e1));
    cJSON_AddStringToObject(resp, "volta", esp_err_to_name(e2));
    return send_json(req, resp);
}
...
/* POST /api/amp {"on":false} -- liga/desliga o amplificador na marra.
 *
 * Diagnostico (2026-08-29): o rele nao estava caindo mesmo com o microfone
 * desligado e nada tocando -- passou de 3 minutos com timeout de 120s. Um
 * amplificador ligado sem sinal chia por conta propria, entao ele e suspeito
 * de ser a fonte do chiado residual que sobra mesmo quando a saida do
 * microfone e zerada. Este endpoint permite testar isso direto, sem esperar
 * timer nenhum. Desligar aqui e temporario: qualquer audio novo religa. */
...
static esp_err_t api_mic_tone_post(httpd_req_t *req)
{
    char buf[48];
    cJSON *root = recv_json_body(req, buf, sizeof(buf));
    if (root == NULL) return ESP_FAIL;
    cJSON *on = cJSON_GetObjectItem(root, "on");
    audio_codec_set_mic_test_tone(cJSON_IsTrue(on));
    cJSON *sil = cJSON_GetObjectItem(root, "silence");
    if (cJSON_IsBool(sil)) {
        audio_codec_set_mic_force_silence(cJSON_IsTrue(sil));
    }
    cJSON_Delete(root);
    cJSON *resp = cJSON_CreateObject();
    cJSON_AddBoolToObject(resp, "ok", true);
...
void web_server_start(void)
{
    ...
    static const httpd_uri_t routes[] = {
        ...
        {.uri = "/api/mic/input", .method = HTTP_POST, .handler = api_mic_input_post},
        ...
        {.uri = "/api/mic/clock", .method = HTTP_POST, .handler = api_mic_clock_post},
        {.uri = "/api/mic/hardreset", .method = HTTP_POST, .handler = api_mic_hardreset_post},
        {.uri = "/api/mic/tone", .method = HTTP_POST, .handler = api_mic_tone_post},
