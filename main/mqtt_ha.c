...
static void handle_command(esp_mqtt_event_handle_t event)
{
    ...
    if (topic_is(event, TOPIC_CMD_MEDIA)) {
        ...
        /* So grava a preferencia -- decidido uma unica vez no boot (ver
         * audio_codec_init()), precisa reiniciar pra valer, igual ao
         * POST /api/config (web_server.c). */
        storage_set_i32(NVS_KEY_MIC_ENABLED, strcmp(payload, "1") == 0 ? 1 : 0);
        logger_log(ESP_LOG_INFO, TAG, "mic_enabled via MQTT: %s (precisa reiniciar)", payload);
