...
esp_err_t audio_codec_mic_set_enabled(bool on)
{
    ...
    if (amostra != NULL) {
        ...
        size_t n_mod = 0;
        /* Descarta o transiente de acordar o codec. */
        for (int b = 0; b < 6 && s_rx_handle != NULL; b++) {
            size_t lidos = 0;
            i2s_channel_read(s_rx_handle, amostra, MIC_LIVE_CHUNK_BYTES, &lidos, pdMS_TO_TICKS(100));
        }
...
static void mic_live_task(void *arg)
{
    ...
    if (s_mic_wanted) {
        ...
        /* RECUPERACAO AUTOMATICA -- e a correcao, nao uma espera.
         *
         * O conversor do ES8388 sobe ruidoso em cerca de uma vez a cada seis,
         * e em 2026-08-30 isso foi isolado ate o fim: nao e configuracao (os
         * 54 registradores sao identicos entre um boot bom e um ruim), nao e a
         * entrada (as quatro dao o mesmo nivel), nao e o PGA (0 a +21dB nao
         * muda nada), nao e o clock (reprograma-lo nao limpa) e nao e estado
         * de registrador (nem o reset total do chip recupera). O ruido nasce
         * dentro do proprio conversor, entre o PGA e o volume digital.
         *
         * A UNICA coisa que tira o chip desse estado e ver o MCLK MORRER --
         * era so isso que o reboot do ESP32 fazia de diferente, e por isso ate
         * agora a unica saida era reiniciar o aparelho e esperar 25 segundos.
         * audio_codec_mic_hard_reset() reproduz exatamente esse corte em 1,5s.
         *
         * Nao e "tentar de novo ate dar certo": e aplicar a acao especifica
         * que corrige a causa, e conferir o resultado. O teto existe porque um
         * defeito diferente nao pode virar laco eterno. */
        for (int t = 0; t < MIC_MAX_HARD_RESETS && s_mic_adc_estado != MIC_ADC_OK; t++) {
            logger_log(ESP_LOG_WARN, TAG,
                       "mic: conversor subiu ruim -- cortando o MCLK pra corrigir (%d de %d)",
                       t + 1, MIC_MAX_HARD_RESETS);
        ...
        if (s_mic_adc_estado != MIC_ADC_OK) {
            logger_log(ESP_LOG_ERROR, TAG,
                       "mic: conversor seguiu ruim apos %d cortes de MCLK -- microfone silenciado",
                       MIC_MAX_HARD_RESETS);
        }
    }
