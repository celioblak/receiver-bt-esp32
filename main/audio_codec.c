...
esp_err_t audio_codec_init(void)
{
    ...
    /* Cala o codec ANTES de o clock nascer. O I2S sobe primeiro de proposito
     * (o codec precisa de MCLK para ser configurado), mas isso deixava uma
     * janela de ~50ms com o ES8388 no reset de fabrica -- desmutado e com as
     * saidas ligadas -- ja recebendo dados. Ver es8388_early_mute(). */
    es8388_early_mute();
...
static void mic_live_task(void *arg)
{
    ...
    for (;;) {
        ...
        /* O mute do DAC acompanha o RELE, nao a palavra.
         *
         * Ele saiu do caminho critico da primeira palavra (era uma ida ao I2C
         * depois do portao abrir, e somava ao atraso que o Celio ouviu), mas
         * na primeira versao disto o DAC passou a ficar desmutado o tempo
         * todo enquanto o microfone estivesse ativo -- e um DAC desmutado com
         * o amplificador ligado deixa passar o ruido de fundo dele proprio,
         * mesmo sem sinal. Trocar por "acompanha o rele" recupera o silencio
         * em repouso sem trazer o atraso de volta: agora quem manda no
         * instante da abertura e o envelope, que responde na primeira amostra,
         * e o unmute e uma escrita de I2C de ~1ms, imperceptivel. */
        if (passar) {
            blocos_sem_voz = 0;
            if (!relay_ligado_por_nos) {
